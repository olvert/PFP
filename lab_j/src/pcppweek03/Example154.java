...
class Example154 {
  public static void main(String[] args) {
    ...
    System.out.println(list2.remove(13));                         // 7 9 0
  }
...
class FunList<T> {
  ...
  /** START – OUR IMPLEMENTION **/

  public FunList<T> remove(T x) { return new FunList<>(recNode(first, x)); }

  private Node<T> recNode(Node<T> n, T x) {

    if (n == null) return null;
    else if (n.item.equals(x)) return recNode(n.next, x);
    else return new Node(n.item, recNode(n.next, x));
  }
  /** END – OUR IMPLEMENTION **/
